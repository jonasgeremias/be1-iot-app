...
(() => {
  ...
  let __defNormalProp = (obj, key, value) =>
    key in obj
      ? __defProp(obj, key, { enumerable: true, configurable: true, writable: true, value })
      : (obj[key] = value);
  let __publicField = (obj, key, value) =>
  ...
  let h = (...args) => getReact().createElement(...args);
  ...
  let RAW_UNWRAP = Object.fromEntries(Object.entries(RAW_WRAP).map(([k, v]) => [v, k]));
  let EVENT_MAP = {
    onclick: 'onClick',
    onchange: 'onChange',
    oninput: 'onInput',
  ...
  let ATTRS = `(?:[^>"']|"[^"]*"|'[^']*')*`;
  let IMPORT_SELF_CLOSE_RE = new RegExp('<(x-import|dc-import)(' + ATTRS + ')/>', 'gi');
  ...
  let BABEL_URL = 'https://unpkg.com/@babel/standalone@7.26.4/babel.min.js';
